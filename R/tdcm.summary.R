...
#'
#' * `transition.probabilities`: conditional attribute proficiency transition
#'   probability matrices
#'
#' * `posterior.probabilities`: examinee marginal attribute posterior
#'   probabilities of proficiency
#'
#' * `transition.posteriors`: examinee marginal attribute transition posterior
#'   probabilities
#'
#' * `most.likely.transitions`: examinee most likely transitions for each
#'   attribute and transition
#'
#' * `classifications`: examinee classifications determined by the specified
#'   threshold applied to the posterior probabilities
#'
#' * `reliability`: estimated transition reliability metrics for each attribute
#'   for the specified transitions. “pt bis” = longitudinal point biserial
#'   metric; “info gain” = longitudinal information gain metric; “polychor” =
#'   longitudinal tetrachoric metric; “ave max tr” = average maximum transition
#'   posterior metric; “P(t>k)” = proportion of examinee marginal attribute
#'   transition posteriors greater than k; “wt pt bis” = weighted longitudinal
#'   point biserial; “wt info gain” = weighted longitudinal information gain.
#'
#' * `att.corr`: estimated attribute correlation matrix
#'
#' * `model.fit`: Several model fit indices and tests are output including item
#'   root mean square error of approximation (RMSEA; von Davier, 2005), mean
#'   RMSEA, bivariate item fit statistics (Chen et al., 2013), and absolute fit
#'   statistics such as mean absolute deviation for observed and expected item
...
#'
#'  DiBello, L. V., Roussos, L. A., & Stout, W. F. (2007). \emph{Review of
#'  cognitively diagnostic assessment and a summary of psychometric models}. In
#'  C. R. Rao and S. Sinharay (Eds.), Handbook of Statistics, Vol. 26
...
#'  \emph{Journal of Educational Measurement, 47}(1), 5 – 31.
#'
#'  Madison, M. J. (2019). Reliably assessing growth with longitudinal
#'  diagnostic classification models. \emph{Educational Measurement: Issues and
#'  Practice, 38}(2), 68-78.
#'
#'  Maydeu-Olivares, A. (2013). Goodness-of-fit assessment of item response
#'  theory models (with discussion). \emph{Measurement: Interdisciplinary
#'  Research and Perspectives, 11}, 71-137.
#'
#'  Schellman, M., & Madison, M. J. (2021, July). \emph{Estimating the
#'  reliability of skill transition in longitudinal DCMs}. Paper presented at
#'  the 2021 International Meeting of the Psychometric Society.
#'
#'  Templin, J., & Bradshaw, L. (2013). Measuring the reliability of diagnostic
#'  classification model examinee estimates. \emph{Journal of Classification,
#'  30}, 251-275.
#'
#'  von Davier M. (2008). A general diagnostic model applied to language testing
#'  data. \emph{The British journal of mathematical and statistical psychology,
#'  61}(2), 287–307.
#'
#' @export
