...
summary.param <- function(model, num.atts, numitems, time.points) {
  ...
  for (k in 1:h.order) { # Set of loops finds combinations for the highest order
    ...
    for (i in 1:nrow(get("temp.param.names1"))) {
      for (t in 1:time.points) {
  ...
  s.parm <- matrix(NA, nrow = numitems, ncol = length(get("param.names1")) + 1) # Creates appropriate empty matrix
  num.count <- nchar(gsub("\\D", "", get("param.names1"))) # Counts the number of integers
  parm.c.names <- c(paste0("\U03BB", 0)) # Sets first column name to lambda0
  for (i in 1:(length(get("param.names1")))) { # This loop goes from 2 to number of parameter names
    for (a in 1:num.atts) {
      if (num.count[i] == a) {
        temp.name <- paste0("\U03BB", a, ",", gsub(", ", "", get("param.names1")[i])) # Creates name lambda1,1 or lambda1,2, or lambda3,123
        parm.c.names <- c(parm.c.names, temp.name)
      } else {}
    }
  }
  colnames(s.parm) <- parm.c.names # Fill in the column names
  rownames(s.parm) <- get("lscoefs0")[, 1] # Fills in the row names using the intercepts which each item should have
  s.parm[, 1] <- get("lscoefs0")[, 3] # Fills in first column with intercepts

  for (t in 1:time.points) {
    assign(paste0("new.p.names", t), c())
  }
  for (i in seq_along(get("param.names1"))) {
    for (t in 1:time.points) {
