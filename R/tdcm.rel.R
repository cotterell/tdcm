...
tdcm.rel <- function(model, numatts, time.points, transition.option, attribute.names=c()){
  ...
  #sample size
  if(model$G == 1){N = model$N} else{N = sum(model$N)}
  ...
  if(transition.option == 1){
    ...
    if(length(attribute.names) == numatts){rnames = attribute.names}
    else{rnames = c(paste("Attribute",1:numatts,sep=" "))}

    cnames = c("pt bis", "pa forms", "info gain", "polychor", "ave max tr",
               "P(t>.6)","P(t>.7)","P(t>.8)","P(t>.9)","wt pt bis", "wt pa forms", "wt info gain")
    mnames = c(paste("T1 to",paste("T",time.points,sep=""), sep=" "))

    if(length(attribute.names) == numatts){tp_mnames = paste(attribute.names,
                                                             c(paste(": T1 to",paste("T",time.points,sep=""), sep=" ")),sep="")}
    else{tp_mnames = paste(c(paste("Attribute",1:numatts,sep=" ")),
                           c(paste(": T1 to",paste("T",time.points,sep=""), sep=" ")),sep="")}
    tp_rnames = c(1:sum(N))
    ...
    tc_rnames = c(1:sum(N))
    if(length(attribute.names) == numatts){tc_cnames = paste(attribute.names,
                                                             c(paste(": T1 to",paste("T",time.points,sep=""), sep=" ")),sep="")}
    else{tc_cnames = paste(c(paste("Attribute",1:numatts,sep=" ")),
                           c(paste(": T1 to",paste("T",time.points,sep=""), sep=" ")),sep="")}
  ...
  #transition.option = 2, first to each
    ...
    tc_rnames = c(1:sum(N))
    if(length(attribute.names) == numatts){s1 = attribute.names}
    else{s1 = c(paste("Attribute",1:numatts,sep=" "))}
  ...
  newList = list("metrics" = round(relresults,3), "transposts" = transposts,
