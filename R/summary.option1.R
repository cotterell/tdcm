...
summary.option1 <- function(model, num.atts, time.points, attribute.names){
  ...
  for(j in 1:num.atts){
    ...
    if(length(attribute.names) == A){temp.name = paste(attribute.names[j], ": Time 1 to Time ", time.points, sep="")}
    else{temp.name = paste("Attribute ", j, ": Time 1 to Time ", time.points, sep="")} #Creates temporary name for each matrix
    matrix.names.trans = c(matrix.names.trans, temp.name) #Combines matrix names into list
  }

  if(length(attribute.names) == A){dimnames(growth) = list(attribute.names, cnames.growth)}
  else{dimnames(growth) = list(rnames.growth, cnames.growth)}
  ...
  if(length(attribute.names) == A){rel = tdcm.rel(model, num.atts, time.points,
                                                  transition.option = transition.option, attribute.names = attribute.names)}
  else{rel = tdcm.rel(model, num.atts, time.points, transition.option = transition.option)}
