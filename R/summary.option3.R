...
summary.option3 <- function(model, num.atts, time.points, attribute.names){
  ...
  for(t in 2:time.points){
    for(j in 1:num.atts){
      ...
      if(length(attribute.names) == A){temp.name = paste(attribute.names[j], ": Time ", (t-1), " to Time ", t, sep="")}
      else{temp.name = paste("Attribute ", j, ": Time ", (t-1), " to Time ", t, sep="")} #Creates temporary name for each matrix
