...
#'
#' @description Function for visualizing results from TDCM analyses.
#'
...
tdcm.plot <- function(results, attribute.names = c(), group.names = c()) {
  ...
  if (results$numgroups == 1) {#single group TDCM plots
    ...
    if (length(attribute.names) == numatts) {
      graphics::legend("topleft", attribute.names, col = 1 + 1:numatts, pch = 16, box.lwd = 0, box.col = 1, bty = "n")
    } else {
      graphics::legend("topleft", c(paste("Attribute", 1:numatts)), col = 1 + 1:numatts, pch = 16, box.lwd = 0, box.col = 1, bty = "n", cex = 1.1)
    }
  ...
  print("**Check plots window for line and bar plots for growth proportions.", quote = FALSE)
